"""UI smoke tests for TimeTrack - runs against live Firebase Hosting URL."""
...
SHOTS = ART / "screenshots-prod"; SHOTS.mkdir(exist_ok=True)
...
def run():
    with sync_playwright() as p:
        ...
        # 7 Network health — allow expected Firebase 400 on invalid login
        print("\n-- 7. Network health --")
        expected_400 = [f for f in failed_reqs if "identitytoolkit" in f or "securetoken" in f]
        unexpected_400 = [f for f in failed_reqs if f not in expected_400]
        rec("No unexpected 4xx/5xx on origin", len(unexpected_400) == 0,
            f"{len(unexpected_400)} unexpected bad responses: {unexpected_400[:3]}")

        # 8 No uncaught JS errors
        print("\n-- 8. JS error hygiene --")
        crit_console = [c for c in console_msgs
                        if c["type"] == "error"
                        and "400" not in c["text"]
                        and "identitytoolkit" not in c["text"]]
        rec("No pageerror exceptions", len(page_errors) == 0,
            f"{len(page_errors)} uncaught: {page_errors[:2]}")
        rec("No unexpected console.error entries", len(crit_console) == 0,
            f"{len(crit_console)} unexpected console errors: {[c['text'][:80] for c in crit_console[:3]]}")
    ...
    (ART/"ui-smoke-prod-results.json").write_text(json.dumps(summary, indent=2))
    print("\n=============================================")
    print(f"UI Smoke (PROD): {summary['passed']}/{summary['total']} passed | "
